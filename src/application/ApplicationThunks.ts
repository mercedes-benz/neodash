          ...
          if (loggingSettings.loggingMode > '0') {
            dispatch(
              createLogThunk(
                ...
                database,
                '',
                `Error while trying to establish connection to Neo4j DB in ${neodashMode
                } mode at ${Date(Date.now()).substring(0, 33)}`
              )
            );
          }
          ...
          if (loggingSettings.loggingMode > '0') {
            dispatch(
              createLogThunk(
                ...
                '',
                `${username
                } established connection to Neo4j DB in ${neodashMode
                } mode at ${Date(Date.now()).substring(0, 33)}`
              )
            );
          }
